...
func initTest(test bool) error {
	if !test {
		...
		err = createTestFile[[]domain.InfoEntity](constants.INFO_DAT_PATH, constants.TEST_PREV_INFO_PATH)
		if err != nil {
			fmt.Println("error in usecase/initprocess/initTest.go:/InitTest/filemanager.LoadAndDecodeJson")
			return err
		}
		err = createTestFile[[]domain.AnswerEntity](constants.ANSWER_DAT_PATH, constants.TEST_PREV_ANSWER_PATH)
		if err != nil {
			fmt.Println("error in usecase/initprocess/initTest.go:/InitTest/filemanager.LoadAndDecodeJson")
			return err
		}
		err = createTestFile[[]domain.BlockEntity](constants.SYNCED_DAT_PATH, constants.TEST_PREV_SYNCED_PATH)
		if err != nil {
			fmt.Println("error in usecase/initprocess/initTest.go:/InitTest/filemanager.LoadAndDecodeJson")
			return err
		}
